...
Developed using:
1) Raspberry Pi Model 3B+ and Pi-Camera
...
The project cost is an estimate which helps in bringing insight about large-scale implementation.
...
The CSV file is converted into Excel file for uploading.
...
Quicksight Analytics dashboard provides user to bring in different formats of data (.csv, .tsv, .xls, .xml etc...)
...
Attendance data can be analysed on AWS Quicksight through various plotting techiques like bar-plots, scatter-plots, heat-maps, pie-charts, linear-plots, donut plots, histograms and many more plots.
...
Considering Post COVID-19 scenario, thermal checking and mask detection are some important add-ons.
